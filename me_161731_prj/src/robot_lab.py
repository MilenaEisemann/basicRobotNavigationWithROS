...
class Robot(object):
    ...
    def __init__(self):
        ...
        #self.subPos = rospy.Subscriber('odom', Odometry, self.modelStates)
        self.subPos = rospy.Subscriber('amcl_pose', PoseWithCovarianceStamped, self.modelStates)
    ...
    def run(self):
        ...
        if self.state == 0:
            ...
            if sqrt(pow(delta_x, 2) + pow(delta_y,2)) < 0.5 and self.regions['frontLeft'] > 0.5 and self.regions['frontRight'] > 0.5:  #diagonal distance to goalpoint --> pythagoras & check if front is clear
                speed.linear.x = 0.0
                speed.angular.z = 0.0
                self.goal_reached()
            else:
            ...
            if (self.regions['frontLeft'] < 0.5 and self.regions['frontLeft'] > 0.0 or self.regions['frontRight'] < 0.5 and self.regions['frontRight'] > 0.0 )and self.move_forward == True:
        ...
        if self.state == 1:
            ...
            if self.following_wall == False:
                ...
                if abs(self.regions['farRight'] - self.nearest <= 0.03):
                    ...
                    print "turning so obstacle is on right"
                    speed.linear.x = 0.0
                    speed.angular.z = 0.7
            ...
            #correct angular.z if too far or too close
                ...
                print self.regions['frontLeft']
                print self.regions['FrontRight']
                if (self.regions['frontLeft'] > 0.5 or self.regions['frontLeft'] == 0.0) and (self.regions['frontRight'] > 0.5 or self.regions['frontRight'] == 0.0):
