...
from math import atan2, sin, cos, pow, sqrt, ceil, pi
...
class Robot(object):
    ...
    def __init__(self):
        ...
        self.Laser = []#360 Laserpoints, 180 per half
        self.LaserInc = pi / 180    #360 Laserscanpoints
        self.index = 0
    ...
    def free_way_to_goal(self, mode):
        if self.turn >= 0.0:
            self.index = int(ceil(self.turn / self.LaserInc))
        elif self.turn < 0.0:
            self.index = int(179+(ceil(self.turn / self.LaserInc)))

        if mode == 0:
            if self.Laser == float('inf') and sqrt(pow(self.inc_x, 2) + pow(self.inc_y, 2) <= 3.5):
                print "I can go straight to the goal"
                return True
            else:
                print "where is the goal?"
                return False
        elif mode == 1:
            if self.Laser == float('inf'):
                return True
            else:
                return False
    ...
    # main
    def run(self):
        self.inc_x = self.goal.x - self.pos.x     #distance robot to goal in x
        self.inc_y = self.goal.y - self.pos.y    #distance robot to goal in y
        speed = Twist()
        ...
        if self.state == 0:
            ...
            self.angle_to_goal = atan2 (self.inc_y, self.inc_x)            #calculate angle through distance from robot to goal in x and y
            dist = sqrt(pow(self.inc_x, 2) + pow(self.inc_y, 2))      #calculate distance
            ...
            if sqrt(pow(self.inc_x, 2) + pow(self.inc_y,2)) < 0.5:  #diagonal distance to goalpoint --> pythagoras
                speed.linear.x = 0.0
                speed.angular.z = 0.0
                self.goal_reached()
            else:
                if abs(self.angle_to_goal - self.theta) < 0.2:    #0.1 because it too exact for a robot if both angles should be exactly 0
            ...
            if self.free_way_to_goal(0):
                ...
                #correct angular.z if too far or too close
                    ...
                    if self.regions['frontLeft'] > 0.7 and self.regions['frontRight'] > 0.7:
                        if self.back_on_track == True and self.dist > 1 and self.free_way_to_goal(1):
                            print "I'm back on track"
                            self.state = 0  # --> switch to new state
                            self.following_wall = False
                        elif self.free_way_to_goal == True: #check if no obstacle in goal direction
                            "I have a free sight towards the goal"
                            self.state = 0
                            self.following_wall = False
                        else:
                            print "adjusting"
                            speed.angular.z = 0.9 * (0.2-self.regions['farRight'])
                            if self.regions['farRight'] >= 99:
                                if self.regions['Right'] < 99:
                                    speed.angular.z = 0.0
